...
<template>
  <div class="w-80 text-white bg-gradient-to-b from-secondary via-secondary-dark to-black backdrop-blur-xl border-l border-primary/20 flex flex-col shadow-2xl">
    
    <!-- Summary Header with gradient -->
    <div class="sticky top-0 z-10 p-4 border-b border-primary/20 bg-gradient-to-r from-primary/20 to-primary-dark/20 backdrop-blur-lg">
      ...
      <div class="flex items-center justify-between mb-2 gap-2">
        ...
        <div class="flex-1 bg-white/5 rounded-lg px-2.5 py-1.5 border border-white/10">
          ...
          <span class="text-base font-bold text-white">{{ orderStore.currentOrder?.order?.order_id || orderStore.currentOrder?.order?.id || sessionStore.orderId || '-' }}</span>
        </div>
      </div>
      
      <!-- Package info (hide in refill mode) -->
      <div v-if="selectedPackage && !isRefillMode" class="flex items-center justify-between bg-white/5 rounded-lg px-3 py-1.5 border border-white/10">
        ...
        <div v-if="!orderStore.hasPlacedOrder" class="flex items-center gap-1.5 bg-white/10 rounded-lg px-2 py-1">
          <button @click="emit('setGuestCount', Math.max(2, guestCount - 1))" class="touch-btn-circle !min-w-[28px] !min-h-[28px] w-7 h-7 text-white font-bold bg-white/10 active:bg-white/30" :disabled="guestCount <= 2">−</button>
          <span class="text-white font-semibold min-w-[2ch] text-center text-sm">{{ guestCount }}</span>
          <button @click="emit('setGuestCount', Math.min(20, guestCount + 1))" class="touch-btn-circle !min-w-[28px] !min-h-[28px] w-7 h-7 text-white font-bold bg-white/10 active:bg-white/30" :disabled="guestCount >= 20">+</button>
        ...
        <div v-if="orderedItems.length > 0">
          ...
          <div class="space-y-2">
            ...
            >
              ...
              <span v-if="item.price > 0" class="text-primary text-xs font-medium">{{ formatCurrency(item.price * item.quantity) }}</span>
      ...
      <template v-else>
        <div v-if="cartItems.length > 0">
          <div class="flex items-center justify-between mb-2">
            ...
            <button @click="cartItems.forEach(item => $emit('removeItem', item.id))" class="text-[10px] text-red-400 hover:text-red-300 active:scale-95 transition-all px-2 py-1 rounded bg-red-500/10">
              Clear All
            </button>
          </div>
          ...
          <div class="space-y-2">
            <cart-item-card
              ...
              :key="item.id"
              :item="item"
              :unlimited-cap="unlimitedItemCap"
        ...
        <el-empty 
          v-else 
          :description="isRefillMode ? 'Add unlimited items for refill' : 'Browse the menu to add items'"
          :image-size="60"
          class="py-6">
          <template #image>
    ...
    <!-- Footer -->
    <div class="sticky bottom-0 border-t border-primary/20 p-4 space-y-3 bg-gradient-to-t from-secondary via-secondary-dark to-transparent backdrop-blur-lg">
      ...
      <div class="space-y-2">
        ...
        <!-- Add Refill Button (after order placed, not in refill mode) -->
        <button
          v-else
