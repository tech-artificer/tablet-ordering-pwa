<template>
    <div class="fixed right-6 top-16 w-1/3 h-[70%] bg-panel rounded-2xl p-4 shadow-xl z-50">
        <div class="flex justify-between items-center mb-4">
            ...
            <FlameButton variant="secondary" @click="$emit('close')">
                Close
            </FlameButton>
        </div>
        ...
        <div class="mt-4">
            <FlameButton :disabled="orderStore.isSubmitting || (orderStore.hasPlacedOrder && !orderStore.isRefillMode)" @click="sendOrder">
                <template v-if="orderStore.isSubmitting">
                    Sending...
                </template>
                <template v-else-if="orderStore.hasPlacedOrder && !orderStore.isRefillMode">
                    Order Placed
                </template>
                <template v-else>
                    Send Order
...
const cart = computed(() => unref(orderStore.draft) as any[])
