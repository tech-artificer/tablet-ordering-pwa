...
const fallbackEmoji = computed(() => {
  ...
  if (cat === 'beverages' || cat === 'DRINK') return '🥤'
...
<template>
  <div class="cart-item group border border-white/10 rounded-2xl overflow-hidden transition-all duration-200 hover:border-primary/25 hover:bg-white/[0.04]"
    style="background: linear-gradient(135deg, rgba(255,255,255,0.055) 0%, rgba(255,255,255,0.02) 100%)">
    <div class="flex items-center gap-3 p-2.5">

      <!-- Thumbnail -->
      <div class="relative flex-shrink-0 w-16 h-16 rounded-xl overflow-hidden"
        ...
        />
        <div class="fallback-icon w-full h-full flex items-center justify-center text-2xl"
          :class="item.img_url ? 'hidden' : ''">
          {{ fallbackEmoji }}
        </div>

        <!-- Unlimited indicator -->
        <div v-if="item.isUnlimited"
          class="absolute bottom-0 inset-x-0 h-1 bg-gradient-to-r from-emerald-500 to-emerald-400"></div>
      </div>

      <!-- Name + Price -->
      <div class="flex-1 min-w-0">
        <p class="text-white font-semibold text-[13px] leading-snug line-clamp-2 mb-0.5">{{ item.name }}</p>
        <div class="flex items-center gap-2">
          ...
          <span v-if="item.isUnlimited"
            class="text-[10px] font-bold uppercase tracking-wider text-emerald-400/80">∞ unlimited</span>
        </div>
      </div>

      <!-- Qty Stepper as compact pill -->
      <div class="flex-shrink-0 flex items-center rounded-xl overflow-hidden border border-white/10"
