    ...
    <div class="max-w-4xl mx-auto pb-12">
      ...
      <div class="mb-8 flex items-center justify-between gap-4">
        <div>
          ...
          <p class="text-white/60">Configure device and connection settings</p>
        </div>
        <div>
          <NuxtLink
            to="/"
      ...
      <div class="bg-white/5 rounded-xl border border-white/10 p-6 mb-6">
        <h2 class="text-2xl font-semibold mb-4 flex items-center gap-2 justify-between">
          ...
          <button @click.prevent="toggle('registration')" class="text-sm text-white/60" aria-label="Toggle Registration">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 transition-transform duration-150" viewBox="0 0 20 20" fill="currentColor"
        ...
        <div v-show="!collapsed.registration" class="space-y-4">
          ...
          <div v-else class="p-4 bg-white/5 rounded-lg border border-white/10">
            <p class="text-sm text-white/60">Device is already registered — see Device Information below.</p>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white/5 rounded-xl border border-white/10 p-6 mb-6">
        <h2 class="text-2xl font-semibold mb-4 flex items-center gap-2 justify-between">
          ...
          <button @click.prevent="toggle('deviceInfo')" class="text-sm text-white/60" aria-label="Toggle Device Information">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 transition-transform duration-150" viewBox="0 0 20 20" fill="currentColor"
          ...
          <div v-if="deviceStore.lastAuthResponse" class="p-4 bg-white/5 rounded-lg border border-white/10">
            ...
            <div class="flex items-start gap-3">
              ...
              <div class="flex flex-col gap-2">
                <button @click="copyToClipboard(JSON.stringify(deviceStore.lastAuthResponse, null, 2))" class="px-3 py-2 rounded bg-primary/20">Copy</button>
                <button @click="deviceStore.lastAuthResponse = null" class="px-3 py-2 rounded bg-white/10">Clear</button>
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center justify-between p-4 bg-white/5 rounded-lg border border-white/10">
            <div>
              ...
              <div class="flex items-center gap-3">
                ...
                <button v-if="displayIsAdmin" @click="editingTable = !editingTable" class="px-2 py-1 text-xs bg-white/5 rounded">{{ editingTable ? 'Cancel' : 'Edit' }}</button>
              </div>
              <div v-if="editingTable" class="mt-3 flex gap-2">
                ...
                <button @click="saveTableOverride()" :disabled="isSavingTable" class="px-3 py-2 rounded bg-primary/20">{{ isSavingTable ? 'Saving...' : 'Save' }}</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="bg-white/5 rounded-xl border border-white/10 p-6 mb-6">
        <h2 class="text-2xl font-semibold mb-4 flex items-center gap-2 justify-between">
          ...
          <div class="flex items-center gap-3">
            <button @click.prevent="toggle('authentication')" class="text-sm text-white/60" aria-label="Toggle Authentication">
              <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 transition-transform duration-150" viewBox="0 0 20 20" fill="currentColor"
        ...
        <div v-show="!collapsed.authentication" class="space-y-4">
          ...
          <div class="p-4 bg-white/5 rounded-lg border border-white/10">
            ...
            <p v-if="tokenMessage" class="text-sm text-white/60">{{ tokenMessage }}</p>
          ...
          <div class="border-t border-white/10 pt-4">
                    <div v-if="!deviceStore.isAuthenticated" class="space-y-3">
                      ...
                      <p v-if="deviceStore.errorMessage" class="text-sm text-red-400">{{ deviceStore.errorMessage }}</p>
                    </div>
      ...
      <div class="bg-white/5 rounded-xl border border-white/10 p-6 mb-6">
        <h2 class="text-2xl font-semibold mb-4 flex items-center gap-2 justify-between">
          ...
          <button @click.prevent="toggle('apiConfig')" class="text-sm text-white/60" aria-label="Toggle API Configuration">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 transition-transform duration-150" viewBox="0 0 20 20" fill="currentColor"
      ...
      <div class="bg-white/5 rounded-xl border border-white/10 p-6 mb-6">
        <h2 class="text-2xl font-semibold mb-4 flex items-center gap-2 justify-between">
          ...
          <button @click.prevent="toggle('display')" class="text-sm text-white/60" aria-label="Toggle Display">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 transition-transform duration-150" viewBox="0 0 20 20" fill="currentColor"
        ...
        <div v-show="!collapsed.display" class="space-y-4">
          <div class="flex items-center justify-between p-4 bg-white/5 rounded-lg border border-white/10">
            <div>
              ...
              <p class="text-xs text-white/60">Toggle kiosk fullscreen for a distraction-free ordering experience.</p>
            </div>
            <div class="flex items-center gap-3">
              <button @click="toggleFullscreen" class="px-4 py-2 rounded-lg bg-primary/20 text-primary border border-primary/30 hover:bg-primary/30 transition-all">
          ...
          <div class="flex items-center justify-between p-4 bg-white/5 rounded-lg border border-white/10">
            <div>
              ...
              <p class="text-xs text-white/60">When enabled, the app will attempt to enter fullscreen on load (may require user permission).</p>
            </div>
            <div>
              <input type="checkbox" id="startFullscreen" v-model="startInFullscreen" @change="saveFullscreenPreference(startInFullscreen)" class="w-5 h-5" />
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="space-y-4">
          ...
          <div v-if="showDiagnostics" class="space-y-4">
            ...
            <div v-if="testOrderPayload" class="p-4 bg-blue-500/10 border border-blue-500/30 rounded-lg">
              <h3 class="text-blue-400 font-semibold mb-2">📤 Request Payload</h3>
            ...
            <div v-if="testOrderResponse" class="p-4 bg-green-500/10 border border-green-500/30 rounded-lg">
              <h3 class="text-green-400 font-semibold mb-2">📥 Response Data</h3>
          ...
          <div class="p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
            <h3 class="text-yellow-400 font-semibold mb-2">💡 How to Debug 500 Errors</h3>
